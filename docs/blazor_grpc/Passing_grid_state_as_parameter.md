...
Some of these examples are showed in the [GridBlazorGrpc project](https://github.com/gustavnavar/Grid.Blazor/tree/master/GridBlazorGrpc.Client)
    ...
    ```razor
        ...
        @code
        {
            ...
            protected override async Task OnParametersSetAsync()
            {
                ...
                var oClient = new GridClient<Order>(gridClientService.GetOrdersGridRows, query, false, "ordersGrid", oColumns)
                _grid = oClient.Grid;
...
* If the grid state parameter is null or empty you must use a new **QueryDictionary<string>**.

The static method **StringExtensions.GetQuery** will convert the string to a **Dictionary<string, StringValues>** object. And the **ToStringDictionary**  method will convert it to a **Dictionary<string, string>** object that will be used in the **GridClient** constructor:
```razor
    @page "/gridsample"
