...
namespace GridCore.Columns
{
    ...
    /// </summary>
    internal class DefaultColumnCoreBuilder<T> : IColumnBuilder<T>
    {
        protected readonly IGridAnnotationsProvider _annotations;
        ...
        public virtual IGridColumn<T> CreateColumn<TDataType>(Expression<Func<T, TDataType>> constraint, IComparer<TDataType> comparer,
            bool hidden)
        {
            bool isExpressionOk = constraint == null || constraint.Body as MemberExpression != null;
            if (isExpressionOk)
            {
                var column = new GridCoreColumn<T, TDataType>(constraint, comparer, _grid);
        ...
        private IGridColumn<T> CreateColumn(PropertyInfo pi, bool hidden)
        {
            ...
            Type columnType = typeof(GridCoreColumn<,>).MakeGenericType(entityType, pi.PropertyType);
