...
{
    public partial class DateOnlyFilterComponent<T>
    {
        ...
        protected ElementReference dateOnlyFilter;
        ...
        {
            get { return GridDateTimeFormats.Placeholder(GridDateTimeFormats.DateType, ColumnPattern); }
        }

        protected string DisplayValue(string value)
        {
            return GridDateTimeFormats.TransportToDisplay(value, GridDateTimeFormats.DateType, ColumnPattern);
        }

        protected void SetDisplayValue(int index, string text)
        {
            _filters[index].Value = GridDateTimeFormats.DisplayToTransport(text, GridDateTimeFormats.DateType, ColumnPattern);
        }
        ...
        protected override void OnParametersSet()
        {
            ...
            if (GridHeaderComponent.GridComponent.Grid.ServerAPI == ServerAPI.OData)
            {
                ...
                for (int i = 0; i < filters.Count(); i++)
                {
                    ...
                    {
                        var dateOnly = DateOnly.Parse(filters[i].FilterValue);
                        filters[i].FilterValue = dateOnly.ToString("yyyy'-'MM'-'dd");
                    }
                }
        ...
        {
            if (!_initPosition && firstSelect.Id != null && dateOnlyFilter.Id != null)
            {
                await jSRuntime.InvokeVoidAsync("gridJsFunctions.focusElement", firstSelect);
                ScreenPosition sp = await jSRuntime.InvokeAsync<ScreenPosition>("gridJsFunctions.getPosition", dateOnlyFilter);
        ...
        protected async Task ApplyButtonClicked()
        {
            ...
            if (GridHeaderComponent.GridComponent.Grid.ServerAPI == ServerAPI.OData)
            {
                ...
                {
                    DateOnly DateOnly = DateOnly.Parse(filter.Value);
                    filter.Value = ODataDateOnlyConverter.ToDateOnlyString(DateOnly);
                }
            }
